...
class T3qNotebookOp(ContainerOp):
    ...
    def __init__(self,
        ...
        if 'arguments' not in kwargs:
            ...
            #################################################################
            # kubeflow내의 artifact로 input과 output을 주고받기 때문에
            # --input argument가 필요없다.
            '''
            ...
            if self.pipeline_outputs:
                ...
                # containerop에서 output artficats에 관여하는 paraemter가 'file_outputs'이다.
                for output_file_pipeline in self.pipeline_outputs:
                    kwargs['file_outputs'][output_file_pipeline.rsplit('.')[0]] = \
                    '/jupyter-work-dir/{}'.format(output_file_pipeline)
                ###################################################################
                ####################################################################
        ...
        ###########################################################################################
        #####환경변수를 inputs.parameter.**의 꼴로 입력하기 위해 있는 부분########################
        # 하지만 processor에서 따로 추가해주기 때문에 필요가 없어져 주석처리했다.
        '''
        if self.pipeline_envs:
            for key, value in self.pipeline_envs.items():  # Convert dict entries to format kfp needs
                ...
                'ELYRA_ENABLE_PIPELINE_INFO','ELYRA_WRITABLE_CONTAINER_DIR']:
        ...
        self.add_pod_label('elyra/node-type',
                           T3qNotebookOp._normalize_label_value(
                               'notebook-script'))
        # Pipeline name
        self.add_pod_label('elyra/pipeline-name',
                           T3qNotebookOp._normalize_label_value(self.pipeline_name))
        # Pipeline version
        self.add_pod_label('elyra/pipeline-version',
                           T3qNotebookOp._normalize_label_value(self.pipeline_version))
        # Experiment name
        self.add_pod_label('elyra/experiment-name',
                           T3qNotebookOp._normalize_label_value(self.experiment_name))
        # Pipeline node name
        self.add_pod_label('elyra/node-name',
                           T3qNotebookOp._normalize_label_value(kwargs.get('name')))
        # Pipeline node file
